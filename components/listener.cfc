<cfcomponent>
    ...
    <cffunction name="logEntry" access="public" returntype="rawEntryBean">
        ...
        <cfscript>
            ...
            var serviceLoader = createObject("component", "service").init( instanceName = variables.instanceName );

            // get handle to bugLogListener service
            var bugLogListener = serviceLoader.getService();

            // create entry bean
            var rawEntry = createObject("component","rawEntryBean")
                                .init()
